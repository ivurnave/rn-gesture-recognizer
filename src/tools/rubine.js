...
function meanGesture(trainingPoints) {
	...
	for (let i = 0; i < pointsLength; i++) {
		...
		retPoint.push(
			new Point(sumX / trainingSetLength, sumY / trainingSetLength),
		);
	}
	return retPoint;
}
...
function frechetHelper(ca, i, j, P, Q) {
	if (ca[i][j] > -1) {
		...
		ca[i][j] = Math.max(
			frechetHelper(ca, i - 1, 0, P, Q),
			Distance(P[i], Q[0]),
		);
	} else if (i == 0 && j > 0) {
		ca[i][j] = Math.max(
			frechetHelper(ca, 0, j - 1, P, Q),
			Distance(P[0], Q[j]),
		);
	} else if (i > 0 && j > 0) {
		ca[i][j] = Math.max(
			Math.min(
				frechetHelper(ca, i - 1, j, P, Q),
				frechetHelper(ca, i - 1, j - 1, P, Q),
				frechetHelper(ca, i, j - 1, P, Q),
			),
			Distance(P[i], Q[j]),
		);
	} else {
...
function frechetDistance(points1, frechetDistanceHelperValues) {
	...
	l2 = points2.length;
	ca = [...Array(l1)].map(e =>
		Array.apply(null, Array(l2)).map(Number.prototype.valueOf, -1),
	);
...
let gestureRecognizer = class {
	...
	train() {
		for (let i = 0; i < this._numClasses; i++) {
			this._avgFeatures.push(
				getAvgFeatureVectorForClass(this._gestureClasses[i]),
			);
		}

		this._invComCovMatrix = math.inv(
			getCommonCovarianceMatrix(this._gestureClasses),
		);
		this._weight0sofAllVectors = getWeight0s(this._gestureClasses);
		this._featureWeightsForClasses = getFeatureWeightsForClasses(
			this._gestureClasses,
		);
	}
	...
	addGestures(trainingData) {
		...
		trainingData.forEach(function(gestureClass) {
			self.addGesture(
				gestureClass.gestureClassName,
				gestureClass.trainingGestures,
				gestureClass.symmetrical,
			);
		});
		this.train();
	}
	...
	addGesture(className, gestureClass, symmetrical) {
		...
		this._frechetDistanceHelperValues.push(
			new FrechetHelpderData(
				meanGesture(trainingGestures),
				deviation,
				symmetrical,
			),
		);
	...
	classifyGesture(points) {
		...
		let frechetD = frechetDistance(
			points,
			this._frechetDistanceHelperValues[maxIndex],
		);
		if (frechetD[1]) {
			return 'Unclassified';
		}
